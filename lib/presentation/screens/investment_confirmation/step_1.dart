...
class _Step1BodyState extends ConsumerState<Step1Body> {
  ...
  Future<void> calculateInvestmentNew(
    ...
    final isSoles = ref.watch(isSolesStateProvider);
    if (formState.amount != 0 && formState.deadline != null) {
      context.loaderOverlay.show();
      final inputCalculator = CalculatorInput(
        amount: formState.amount,
        months: formState.deadline!.value,
      ...
      child: SingleChildScrollView(
        child: Column(
          children: <Widget>[
            ...
            ContainerForm(
              calculate: calculateInvestmentNew(
                context,
                ref,
              ),
            ),
...
class ContiniuButton extends ConsumerWidget {
  const ContiniuButton({
    super.key,
    required this.userProfile,
    required this.ref,
    required this.widget,
    required this.deadLineFuture,
  ...
  final WidgetRef ref;
  final Step1Body widget;
  final Future<List<DeadLineEntity>> deadLineFuture;
  final Future<List<BankEntity>> bankFuture;
  final bool isSoles;
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return TextButton(
      onPressed: () async {
        ...
        //   formState.uuidDeadline.toString(),
